...
- **`src/engine/share.ts`** — cross-device sharing of the identified list. `encodeShare()` packs item indices into a base64url bitset (`1.<gameId>.<dungeonId>.<bits>`), carried in the URL hash `#s=...` and shown as a QR code (`qrcode-generator`). `App.tsx` reads the hash on mount and shows `ImportDialog` (overwrite / merge), then clears the hash.
